...
Pic* loadTexture(char* texFile)
{
  int nx, ny;
  if(ppm_get_size(texFile, &nx, &ny))
  {
     return ppm_read(texFile,pic_alloc(nx,ny,3,NULL));
  }
  return NULL;
}
...
void showSkyBox(){
  ...
  Pic* image = loadTexture("../skyboxes/sky_purple.ppm");
  if(image == NULL){
