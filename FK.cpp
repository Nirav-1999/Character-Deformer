    ...
    vector<RigidTransform4d> & localTransforms, vector<RigidTransform4d> & globalTransforms)
{
  ...
  int numberOfJoints;
  numberOfJoints = jointParents.size();
  double rotationMatrix[9];
  Mat3d rotationMatrix3d;
  ...
  for (int i = 0; i < numberOfJoints; i++) {
    ...
    int currentJoint = jointUpdateOrder[i];
    // Get the rotation matrix from the joint orientation i.e. orientation of the joint w.r.t. the parent joint
    euler2Rotation(jointOrientationEulerAngles[currentJoint], rotationMatrix, RotateOrder::XYZ);
    rotationMatrix3d = Mat3d(rotationMatrix);
    // Get the rotation matrix from the current joint's rotation
    euler2Rotation(eulerAngles[currentJoint], rotationMatrix, rotateOrders[currentJoint]);
    
    // Final rotation = joint orientation * current joint rotation
    rotationMatrix3d = rotationMatrix3d * Mat3d(rotationMatrix);

    // Set the local transform for the current joint using the rotation matrix and the translation
    localTransforms[currentJoint] = RigidTransform4d(rotationMatrix3d, translations[currentJoint]);
    ...
    if (jointParents[currentJoint] == -1) {
      globalTransforms[currentJoint] = localTransforms[currentJoint];
    }
    // Otherwise, the global transform is the global transform of the parent joint multiplied by the local transform of the current joint
      ...
      globalTransforms[currentJoint] = globalTransforms[jointParents[currentJoint]] * localTransforms[currentJoint];
    }
  }
    ...
    vector<RigidTransform4d> & skinTransforms)
{
  // Students should implement this.
  int numberOfJoints;
  numberOfJoints = globalTransforms.size();
